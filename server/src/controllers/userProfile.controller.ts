...
// Adjust this to match your auth middleware's shape
...
/**-----------------------------------------
    Zod schema for profile PATCH payload
--------------------------------------------*/
...
function encodePullingFrequency(raw?: string): number | undefined {
    ...
    if (v.includes("several") || v.includes("few") || v.includes("times a week"))
        return 4;
...
function normalizeStopped(
    ...
    if (typeof raw === "string") {
        ...
        // unknown string → keep raw, no encoded
        return { raw };
    }
...
/**-----------------------------------------
    Controller: PATCH /api/users/profile
--------------------------------------------*/
export async function updateProfile(
    ...
    try {
        ...
        // Derive how_long_stopped_days_est from raw days if present
        if (typeof data.how_long_stopped_days !== "undefined") {
            update.how_long_stopped_days_est = Math.max(
                0,
                data.how_long_stopped_days
            );
        }

        // Encoded frequency & awareness
        if (typeof data.pulling_frequency !== "undefined") {
            const encFreq = encodePullingFrequency(
                data.pulling_frequency
            );
        ...
        // Convert _id → id to match ExtendedUser on the client
