...
export const PredictDTO = z.object({
    age: z.coerce.number().min(0).max(120).default(30),
    age_of_onset: z.coerce.number().min(0).max(120).default(15),
    years_since_onset: z.coerce.number().min(0).default(0),
    ...
    successfully_stopped_encoded: z.coerce.number().int().min(0).max(1).default(0),
    how_long_stopped_days_est: z.coerce.number().min(0).default(0),
