/**
 * server.prod.ts — TrichMind Production Server
 *
 * Runs Express optimized for production deployment.
 * Includes: Helmet, Compression, CORS, MongoDB, Error Handling, Logging
 */
...
connectMongo().then(() => {
    app.listen(ENV.PORT, "0.0.0.0", () => {
        logger.info(`🚀 TrichMind Production Server running on port ${ENV.PORT}`);
    });
});
