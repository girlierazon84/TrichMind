...
async function rawGetProfile():
    ...
    const res =
        await axiosClient.get<GetProfileResponse>(
            `${USERS_BASE}/profile`
        );
...
async function rawUpdateProfile(
    data: UpdateProfileData
): Promise<UpdateProfileResponse> {
    // Make a PATCH request to the profile endpoint with the provided data
    const res =
        await axiosClient.patch<UpdateProfileResponse>(
            `${USERS_BASE}/profile`,
            data
        );
...
async function rawDeleteAccount():
    ...
    const res =
        await axiosClient.delete<DeleteAccountResponse>(
            `${USERS_BASE}/delete`
        );
